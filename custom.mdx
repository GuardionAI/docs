...
3. **Set sensibility** per guardrail — adjust the confidence threshold for each guardrail independently
4. **Assign the policy** to one or more applications

This allows you to build layered defense strategies — for example, a single policy that checks for prompt injections, PII exposure, and your custom compliance rules all at once, each with its own sensibility level.
